...
class ItemBellChristmas : ItemAoeWeaponBase() {
    ...
    val damage = status("damage", { (3.0 + (!EnumManaType.SHINE + !EnumErgType.CHRISTMAS) / 10.0) * costFactor }, { float2 })
    val damageBoost = status("damageBoost", { 1.0 + !EnumMastery.rangedCombat / 100.0 }, { percent0 })

    override val additionalReach = status("additionalReach", { 3.0 + (!EnumManaType.WIND + !EnumErgType.LEVITATE) / 10.0 atMost 30.0 }, { float2 })
    override val radius = status("radius", { 4.0 + (!EnumManaType.GAIA + !EnumErgType.SOUND) / 20.0 atMost 10.0 }, { float2 })
    override val maxTargetCount = status("maxTargetCount", { floor(5.0 + (!EnumManaType.GAIA + !EnumErgType.SPACE) / 10.0).toInt() }, { integer })

    override val looting = status("looting", { 3.0 + (!EnumManaType.DARK + !EnumErgType.SUBMISSION) / 30.0 }, { float2 })
