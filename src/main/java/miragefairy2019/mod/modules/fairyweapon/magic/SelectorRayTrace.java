...
public class SelectorRayTrace
{
	...
	public SelectorRayTrace(World world, EntityPlayer player, double additionalReach)
	{
		...
		this.position = oRayTraceResult.isPresent()
			? oRayTraceResult.get().hitVec
			: getSight(player, player.getEntityAttribute(EntityPlayer.REACH_DISTANCE).getAttributeValue() + additionalReach);
	}
