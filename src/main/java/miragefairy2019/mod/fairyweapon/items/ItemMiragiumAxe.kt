...
class ItemMiragiumAxe(private val baseSpeed: Double, private val baseRange: Double, private val baseFortune: Double) : ItemMiragiumToolBase() {
    ...
    override fun getAdditionalReach(magicArguments: MagicArguments) = additionalReach(magicArguments)
    ...
    override fun iterateTargets(magicArguments: MagicArguments, blockPosBase: BlockPos) = iterator {
        ...
        val stemResult = treeSearch(
            magicArguments.world,
            listOf(blockPosBase),
            mutableSetOf(),
            maxDistance = range(magicArguments),
            maxSize = 1000,
            includeZero = true,
            neighborhoodType = NeighborhoodType.VERTEX,
            tooLargeBehaviour = TooLargeBehaviour.IGNORE
        ) { it, _ -> isLog(magicArguments.world.getBlockState(it)).toUnitOrNull() }
        ...
        val leavesResult = treeSearch(
            ...
            maxSize = 2000,
            tooLargeBehaviour = TooLargeBehaviour.IGNORE
        ) { it, _ -> isLeaves(magicArguments.world.getBlockState(it)).toUnitOrNull() }
